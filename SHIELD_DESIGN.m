...
%Coil windings should be parallel to y - if needed, swap Jy and Jx to
%rotate
JX = reshape(Jy,size(X));
JY = reshape(Jx,size(X));


J_Abs = sqrt(JX.^2 + JY.^2);


%ENTER SHIFTS (specific for current dataset)
% X=X-5; 
Y = Y - 8;
...
%Get rid of points that overlap into negative x
for i = 1:length(streamLineList)
    streamLine = streamLineList{i};
