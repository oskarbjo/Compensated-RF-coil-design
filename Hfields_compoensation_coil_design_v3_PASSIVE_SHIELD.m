...
%Find x = 0 index
[rowX,colX,dat]=find(X==0);
zero_ind = colX(end); 

% Calculate current integrals (normalize to zero) and smooth
Nsmooth=8;
JX_lineDensity = movmean(JX(:,zero_ind)+abs(sum(JX(:,zero_ind)))/length(JX(:,zero_ind)),Nsmooth);
JY_lineDensity = movmean(JY(:,zero_ind)+abs(sum(JY(:,zero_ind)))/length(JY(:,zero_ind)),Nsmooth);
...
%Segment which should hold a quarter of integral(abs(JX_lineDensity))
...
hold off;
offsetY = [0,0,0,0,0.2,0.3,0.5,0,0,0,0,0,0,0]; %ADJUST ACCORDING TO DATA SET
for i = 1:length(streamLineList)
    streamLineList{i}(:,2) = streamLineList{i}(:,2) + offsetY(i);
    XY=streamLineList{i};
...
maxWidth = 0.12; %[mm], adjust widths according to each trace length
